(() => {
  ...
  const mount = () => {
    ...
    section.querySelector('[data-open-issue]').addEventListener('click', () => {
      ...
      dialog.innerHTML = `<form method="dialog" style="padding:20px"><h2 style="margin:0 0 8px">แจ้งปัญหาระหว่างส่ง</h2><p class="mpa-muted">ทีม Dispatch จะเห็นรายงานนี้ทันที เลือกเหตุผลแบบสั้นเพื่อช่วยดำเนินการเร็วขึ้น</p><label class="mpa-field"><span>ประเภทปัญหา</span><select name="issue_type" required>${issueOptions.map(([value, label]) => `<option value="${value}">${label}</option>`).join('')}</select></label><label class="mpa-field"><span>รายละเอียดเพิ่มเติม</span><textarea name="detail" rows="3" maxlength="500" placeholder="จำเป็นเมื่อเลือกอื่น ๆ"></textarea></label><label class="mpa-field"><span>รูปหลักฐาน (ไม่บังคับ)</span><input name="evidence" type="file" accept="image/jpeg,image/png,image/webp" capture="environment"><span class="mpa-media-preview" data-issue-preview hidden><img alt="ตัวอย่างรูปหลักฐานที่เลือก"><small data-issue-preview-status></small></span></label><p data-issue-status class="mpa-muted" aria-live="polite">รูปจะถูกบีบอัดเป็นไฟล์ private ไม่เกิน 1 MB ก่อนส่ง</p><div style="display:flex;justify-content:flex-end;gap:8px;margin-top:12px"><button type="button" data-close class="mpa-button mpa-button-secondary">ยกเลิก</button><button type="submit" class="mpa-button" style="background:#b42318">ส่งรายงาน</button></div></form>`;
      document.body.append(dialog); dialog.showModal(); let issuePreviewUrl = ''; const issueInput = dialog.querySelector('input[name="evidence"]'); const issuePreview = dialog.querySelector('[data-issue-preview]'); const clearIssuePreview = () => { if (issuePreviewUrl) { URL.revokeObjectURL(issuePreviewUrl); issuePreviewUrl = ''; } if (issuePreview) { issuePreview.hidden = true; issuePreview.querySelector('img').removeAttribute('src'); issuePreview.querySelector('[data-issue-preview-status]').textContent = ''; } }; issueInput.addEventListener('change', () => { const selected = issueInput.files?.[0]; clearIssuePreview(); if (selected) { issuePreviewUrl = URL.createObjectURL(selected); issuePreview.hidden = false; issuePreview.querySelector('img').src = issuePreviewUrl; issuePreview.querySelector('[data-issue-preview-status]').textContent = `${selected.name} · ${Math.ceil(selected.size / 1024)} KB`; } }); dialog.addEventListener('close', clearIssuePreview, { once: true });
