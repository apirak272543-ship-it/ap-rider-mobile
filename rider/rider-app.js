(() => {
  ...
  async function delivery() {
    ...
    try {
      ...
      const statusNames = { [C.contracts.orderStatus.RIDER_PICKUP]: 'ไปรับสินค้า', [C.contracts.orderStatus.ARRIVED_STORE]: 'ถึงร้านค้า', [C.contracts.orderStatus.COLLECTED]: 'รับสินค้าแล้ว', [C.contracts.orderStatus.DELIVERING]: 'ออกไปส่ง', [C.contracts.orderStatus.COMPLETED]: 'ส่งสำเร็จ' };
      const currentIndex = riderSteps.indexOf(job.status);
      const nextStep = steps.find(status => status !== C.contracts.orderStatus.CANCELLED) || '';
