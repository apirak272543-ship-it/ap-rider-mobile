(() => {
  ...
  async function login() {
    document.body.innerHTML = `<main class="ap-login-shell"><section class="ap-login-card" data-login-panel="rider"><div class="ap-login-brandline"><span class="ap-login-mark">AS</span><div><strong>AP Service</strong><small>ศูนย์จัดการงานส่งและสถานะ Rider</small></div></div><span class="ap-login-role">Rider Login</span><h1 class="ap-login-title">เข้าสู่ระบบ Rider</h1><p class="ap-login-intro">ใช้บัญชีที่ Admin สร้างและผูกสิทธิ์กับโปรไฟล์ Rider แล้ว</p><form id="login" class="ap-login-form"><label class="ap-login-field"><span>ชื่อผู้ใช้ Rider</span><div class="ap-login-control"><span class="ap-login-icon" aria-hidden="true">${window.APLoginUI?.icon('user') || window.APLoginUI?.icon('mail') || ''}</span><input id="username" type="text" autocomplete="username" autocapitalize="none" spellcheck="false" aria-label="ชื่อผู้ใช้ Rider" placeholder="เช่น rider01" required></div></label><label class="ap-login-field"><span>รหัสผ่าน</span><div class="ap-login-control"><span class="ap-login-icon" aria-hidden="true">${window.APLoginUI?.icon('lock') || ''}</span><input id="password" type="password" autocomplete="current-password" aria-label="รหัสผ่าน" placeholder="กรอกรหัสผ่านของคุณ" required><button class="ap-login-password-toggle" type="button" data-password-toggle aria-controls="password" aria-label="แสดงรหัสผ่าน">${window.APLoginUI?.icon('eye') || ''}</button></div></label><button class="ap-login-submit" data-login-submit type="submit">เข้าสู่ระบบ</button><p class="ap-login-status" data-login-status aria-live="polite"></p></form><div class="ap-login-admin-note"><span aria-hidden="true">${window.APLoginUI?.icon('shield') || ''}</span><div><strong>บัญชีนี้สร้างโดย Admin เท่านั้น</strong><p>หากยังไม่มีบัญชี โปรดให้ผู้ดูแลระบบสร้างและผูกสิทธิ์ก่อนเข้าสู่ระบบ</p></div></div><a class="ap-login-back" href="login.html" aria-label="อยู่ที่หน้าเข้าสู่ระบบ Rider">กลับหน้าเข้าสู่ระบบ Rider</a></section></main>`;
    const loginForm = $('#login');
    window.APLoginUI?.enhance(loginForm);
    loginForm.onsubmit = async event => { event.preventDefault(); const username = $('#username').value.trim(), password = $('#password').value; if (!username || !password) { const message = 'กรุณากรอกชื่อผู้ใช้และรหัสผ่านให้ครบ'; window.APLoginUI?.showError(loginForm, message); M.ui.setNotice(message, 'error'); return; } try { await M.auth.signInWithUsername(username, password, 'rider'); await window.APLoginUI?.showSuccess(loginForm); location.assign('dashboard.html'); } catch (error) { const raw = String(error?.message || ''); const message = /missing username|missing identifier|missing password|invalid login|invalid credentials|ชื่อผู้ใช้หรือรหัสผ่าน/i.test(raw) ? 'ชื่อผู้ใช้หรือรหัสผ่านไม่ถูกต้อง กรุณาตรวจสอบแล้วลองใหม่' : raw; window.APLoginUI?.showError(loginForm, message); M.ui.setNotice(message || 'เข้าสู่ระบบ Rider ไม่สำเร็จ กรุณาลองใหม่อีกครั้ง', 'error'); } };
  }
