(() => {
  ...
  const mount = (job, options = {}) => {
    ...
    void renderMap();
    $('#riderVerifyGps').onclick = async () => { const button = $('#riderVerifyGps'); button.disabled = true; setStatus('กำลังอ่านตำแหน่งจากอุปกรณ์…'); try { state.current = await gps(); const accuracy = Math.round(state.current.accuracy || 0); setArrivalState(state.current); startArrivalWatch(); setStatus(`พบตำแหน่งของคุณแล้ว · ความแม่นยำประมาณ ${accuracy} เมตร`, 'success'); await renderMap(); } catch (error) { setArrivalState(null, false); setStatus(error.message, 'warning'); setMapStatus('ยังใช้ตำแหน่งปัจจุบันไม่ได้ แผนที่ยังแสดงจุดหมายจากข้อมูล order', 'warning'); M.ui.setNotice(error.message, 'error'); } finally { button.disabled = false; } };
    $('#riderConfirmArrival').onclick = () => { if (!state.arrivalReady) return M.ui.setNotice('ระบบยังยืนยันไม่ได้ว่าคุณถึงร้าน กรุณาตรวจ GPS แล้วลองใหม่ หรือใช้การยืนยันด้วยตนเอง', 'error'); const button = $('#riderConfirmArrival'); button.disabled = true; stopArrivalWatch(); options.onArrivalConfirmed?.({ jobId: job?.id, distance: distanceMeters(state.current, pickup), accuracy: state.current?.accuracy, location: state.current, manual: false }); };
    $('#riderManualArrival').onclick = () => { if (!confirm('ยืนยันว่าคุณอยู่หน้าร้านค้าแล้วใช่หรือไม่?')) return; const button = $('#riderManualArrival'); button.disabled = true; stopArrivalWatch(); options.onArrivalConfirmed?.({ jobId: job?.id, distance: null, accuracy: null, location: null, manual: true }); };
    $('#riderCenterMap').onclick = () => { if (!state.map) return; const point = state.current || target || pickup || destination; if (point) state.map.setView([point.lat, point.lng], 16); };
    $('#riderOpenManualMap').onclick = async () => { const rawPoint = { lat: $('#riderManualLat')?.value, lng: $('#riderManualLng')?.value }; if (!validPoint(rawPoint)) return M.ui.setNotice('กรุณากรอก Latitude และ Longitude ให้ถูกต้อง', 'error'); const point = toPoint(rawPoint); state.current = point; setArrivalState(state.current, false); setStatus(`ใช้พิกัดอ้างอิง ${state.current.lat.toFixed(6)}, ${state.current.lng.toFixed(6)} เพื่อจัดแผนที่`, 'success'); await renderMap(); };
