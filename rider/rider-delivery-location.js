(() => {
  ...
  const mount = job => {
    ...
    $('#riderOpenManualMap').onclick = () => { const rawPoint = { lat: $('#riderManualLat')?.value, lng: $('#riderManualLng')?.value }; if (!validPoint(rawPoint)) return M.ui.setNotice('กรุณากรอก Latitude และ Longitude ให้ถูกต้อง', 'error'); const point = toPoint(rawPoint); const urls = maps(point); window.open(urls.google, '_blank', 'noopener'); setStatus(`เปิดพิกัดอ้างอิง ${point.lat.toFixed(6)}, ${point.lng.toFixed(6)} ในแผนที่แล้ว`, 'success'); };
  };
