...
assert.match(nativeApi, /action: "update_rider_presence"/, 'Rider native/WebView API must share the same server presence action');
assert.doesNotMatch(nativeApi, /rest\(`riders\?id=eq\.\$\{encodeURIComponent\(rider\.id\)\}`, session, \{ method: "PATCH"/, 'Rider native/WebView API must not direct-patch location or readiness');
